...
```
fnowallet --create --createwatchingonly
```
...
granted that fnod and fnowallet are installed and vendored dependencies 
are up to date with dep:

```
cd $GOPATH/src/github.com/fnowallet/cmd/movefunds
go install
```
...
1. On the machine with the watching only wallet, call 'listunspent' and pipe 
    the output to unspent.json (fnoctl --wallet listunspent > unspent.json). 
	Next, run:
	```
	fnoctl --wallet accountaddressindex myAccountName 0
	fnoctl --wallet accountaddressindex myAccountName 1
	```
...
3. Open a terminal and change directory to where unspent.json is. Then, copy 
    config.json from $GOPATH/src/github.com/fnowallet/cmd/movefunds to 
	this directory.
	```
	cp $GOPATH/src/github.com/fnowallet/cmd/movefunds/config.json config.json
	```
...
6. Connect fnowallet on the cold machine. Synchronize the addresses on this 
    wallet using the command and the responses you got at step 1:
	```
	fnoctl --wallet accountsyncaddressindex myAccountName 0 <response1>
	fnoctl --wallet accountsyncaddressindex myAccountName 1 <response2>
	```
...
8. Send the raw transaction on the hot wallet machine.
    ```
	fnoctl sendrawtransaction $(cat rawtx.txt)
    ```
