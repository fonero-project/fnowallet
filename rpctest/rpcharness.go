...
import (
	...
	rpc "github.com/fonero-project/fnod/rpcclient"
)
...
// Harness fully encapsulates an active fnod process, along with an embedded
// fnowallet to provide a unified platform for creating RPC-driven integration
// tests involving fnod. The active fnod node will typically be run in simnet
...
// spends. The active fnod process is fully managed by Harness, which handles
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// Start the fnod node itself. This spawns a new process which will be
	// managed
	...
	// Start fnowallet. This spawns a new process which will be managed
	...
	// Get a new address from the wallet to be set with fnod's --miningaddr
	time.Sleep(5 * time.Second)
	var miningAddr fnoutil.Address
	for i := 0; i < 100; i++ {
...
// fnod process belonging to this Harness instance. If the initial connection
