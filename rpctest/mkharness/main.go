...
import (
	...
	"github.com/fonero-project/fnowallet/rpctest"
)
...
func main() {
	...
	fmt.Println("Command for node's fnoctl:")
	fmt.Printf("\tfnoctl -u %s -P %s -s %s -c %s\n", cn.User, cn.Pass,
		cn.Host, nodeCertFile)

	cw := primaryHarness.RPCWalletConfig()
	walletCertFile := primaryHarness.RPCWalletCertFile()
	fmt.Println("Command for wallet's fnoctl:")
	fmt.Printf("\tfnoctl -u %s -P %s -s %s -c %s --wallet\n", cw.User, cw.Pass,
