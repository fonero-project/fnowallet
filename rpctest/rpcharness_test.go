...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := fnorpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = fnoutil.DecodeAddress(addrStr)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := fnoutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := fnoutil.DecodeAddress(devSubAddrStr)
	if err != nil {
...
func testWalletPassphrase(r *Harness, t *testing.T) {
	...
	if err != nil && err.(*fnojson.RPCError).Code !=
		fnojson.ErrRPCWalletPassphraseIncorrect {
		// fnojson.ErrWalletPassphraseIncorrect.Code
	...
	// fnojson.ErrRPCWalletUnlockNeeded
	if !strings.HasPrefix(err.Error(),
		strconv.Itoa(int(fnojson.ErrRPCWalletUnlockNeeded))) {
		t.Fatalf("createnewaccount returned error (%v) instead of %v",
			err, fnojson.ErrRPCWalletUnlockNeeded)
	}
	...
	if err != nil && err.(*fnojson.RPCError).Code !=
		fnojson.ErrRPCWalletAlreadyUnlocked {
		t.Fatalf("WalletPassphrase failed: %v", err)
	}
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	preAccountBalances := make(map[string]fnojson.GetAccountBalanceResult)
	...
	sendAmount := fnoutil.Amount(700000000)
	...
	postAccountBalances := make(map[string]fnojson.GetAccountBalanceResult)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := fnoutil.Amount(700000000)
	...
	// Get *fnoutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = fnoutil.Amount(txIn.ValueIn).ToCoin()
	}
	...
	for txinID, amt := range txInIDs {
		if _, ok := utxosBeforeSend[txinID]; !ok {
			t.Fatalf("Failed to find txid %v (%v FNO) in list of UTXOs",
				txinID, amt)
		}
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := fnoutil.Amount(1000000)
	...
	fee := fnoutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []fnoutil.Amount{700000000, 1400000000}
	addresses := []fnoutil.Address{}
	...
	addressAmounts := make(map[fnoutil.Address]fnoutil.Amount)
	totalAmountToSend := fnoutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	if *txList1[0].TxType != fnojson.LTTTRegular {
	...
	voutAmt := fnoutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := fnoutil.Amount(240000000)
	...
	var sendResult, recvResult fnojson.ListTransactionsResult
	...
	rxtxResults := map[string]fnojson.ListTransactionsResult{
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// fnorpcclient does not have a getwalletfee or any direct method, so we
	...
	origTxFee, err := fnoutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := fnoutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := fnoutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(fnoutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// fnorpcclient does not have a getticketee or any direct method, so we
	...
	origTicketFee, err := fnoutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := fnoutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := fnoutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := fnoutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(fnoutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := fnoutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := fnoutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func mustGetStakeInfo(wcl *fnorpcclient.Client, t *testing.T) *fnojson.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *fnoutil.Block, []*chainhash.Hash) {
...
func newBlockAtQuick(currentHeight uint32, r *Harness,
	...
	return block.Header.Height, fnoutil.NewBlock(block), blockHashes
}

func getBestBlock(r *Harness, t *testing.T) (uint32, *fnoutil.Block, *chainhash.Hash) {
	...
	return curBlockHeight, fnoutil.NewBlock(bestBlock), bestBlockHash
}
...
func includesTx(txHash *chainhash.Hash, block *fnoutil.Block) bool {
	if len(block.Transactions()) <= 1 {
		return false
	}
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *fnoutil.Block) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *fnoutil.Tx) fnoutil.Amount {
	var totalSpent int64
	...
	return fnoutil.Amount(totalSpent - totalSent)
}

// getOutPointString uses OutPoint.String() to combine the tx hash with vout
// index from a ListUnspentResult.
func getOutPointString(utxo *fnojson.ListUnspentResult) (string, error) {
