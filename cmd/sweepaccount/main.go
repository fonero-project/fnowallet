...
var (
	walletDataDirectory = fnoutil.AppDataDir("fnowallet", false)
	newlineBytes        = []byte{'\n'}
)
...
func makeInputSource(outputs []fnojson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue   fnoutil.Amount
		inputs            = make([]*wire.TxIn, 0, len(outputs))
		redeemScriptSizes = make([]int, 0, len(outputs))
		sourceErr         error
	)
	for _, output := range outputs {
		outputAmount, err := fnoutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(fnoutil.Amount) (*txauthor.InputDetail, error) {
...
func (src *destinationScriptSourceToAddress) Script() ([]byte, uint16, error) {
	destinationAddress, err := fnoutil.DecodeAddress(src.address)
	if err != nil {
		return nil, 0, err
	}
	script, err := txscript.PayToAddrScript(destinationAddress)
...
func sweep() error {
	...
	rpcClient, err := fnorpcclient.New(&fnorpcclient.ConnConfig{
	...
	sourceOutputs := make(map[string][]fnojson.ListUnspentResult)
	...
	feeRate, err := fnoutil.NewAmount(opts.FeeRate)
	if err != nil {
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := fnoutil.Amount(atx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount fnoutil.Amount) bool {
	return amount >= 0 && amount <= fnoutil.MaxAmount
}

func parseOutPoint(input *fnojson.ListUnspentResult) (wire.OutPoint, error) {
