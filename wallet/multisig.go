...
// from them.
func (w *Wallet) MakeSecp256k1MultiSigScript(secp256k1Addrs []fnoutil.Address, nRequired int) ([]byte, error) {
	const op errors.Op = "wallet.MakeSecp256k1MultiSigScript"

	secp256k1PubKeys := make([]*fnoutil.AddressSecpPubKey, len(secp256k1Addrs))
	...
	for i, addr := range secp256k1Addrs {
		...
		case *fnoutil.AddressPubKeyHash:
			if addr.DSA(w.chainParams) != fnoec.STEcdsaSecp256k1 {
				return nil, errors.E(op, errors.Invalid, "address key is not secp256k1")
			}
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*fnoutil.AddressScriptHash, error) {
	const op errors.Op = "wallet.ImportP2SHRedeemScript"

	var p2shAddr *fnoutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if errors.Is(errors.Exist, err) {
				...
				p2shAddr, _ = fnoutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*fnoutil.AddressScriptHash)
		return nil
	})
	if err != nil {
...
func (w *Wallet) FetchP2SHMultiSigOutput(outPoint *wire.OutPoint) (*P2SHMultiSigOutput, error) {
	...
	p2shAddr, err := fnoutil.NewAddressScriptHashFromHash(
