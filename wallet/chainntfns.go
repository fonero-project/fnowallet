...
func (w *Wallet) evaluateStakePoolTicket(rec *udb.TxRecord, blockHeight int32, poolUser fnoutil.Address) bool {
	...
	out := fnoutil.Amount(0)
	for i := range tx.TxOut {
		out += fnoutil.Amount(tx.TxOut[i].Value)
	}
	fees := in - out
	...
	if exists {
		...
		feeNeeded := txrules.StakePoolTicketFee(fnoutil.Amount(
...
func (w *Wallet) processTransactionRecord(dbtx walletdb.ReadWriteTx, rec *udb.TxRecord,
	...
	if stake.IsSStx(&rec.MsgTx) {
		...
		if insert {
			err := w.StakeMgr.InsertSStx(stakemgrNs, fnoutil.NewTx(&rec.MsgTx))
			if err != nil {
				...
				case err == nil:
					if n, err := w.NetworkBackend(); err == nil {
						...
						err := n.LoadTxFilter(context.TODO(),
							false, []fnoutil.Address{addr}, nil)
						if err != nil {
							return nil, errors.E(op, err)
						}
					}
...
func (w *Wallet) VoteOnOwnedTickets(winningTicketHashes []*chainhash.Hash, blockHash *chainhash.Hash, blockHeight int32) error {
	...
	if err != nil {
		...
		rpcErr, ok := err.(*fnojson.RPCError)
		if !ok || rpcErr.Code != fnojson.ErrRPCDuplicateTx {
...
func (w *Wallet) RevokeOwnedTickets(missedTicketHashes []*chainhash.Hash) error {
	...
	err = walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, ticketHash := range ticketHashes {
			...
			err = w.checkHighFees(fnoutil.Amount(ticketPurchase.TxOut[0].Value), revocation)
			if err != nil {
