...
func p2pkhOutputs(amounts ...fnoutil.Amount) []*wire.TxOut {
...
func TestNewUnsignedTransaction(t *testing.T) {
	...
	for i, test := range tests {
		...
		if tx.ChangeIndex < 0 {
			...
			changeAmount := fnoutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
			if test.ChangeAmount == 0 {
