...
type InputSource func(target fnoutil.Amount) (detail *InputDetail, err error)
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb fnoutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, secrets SecretsSource) error {
	...
	for i := range inputs {
		...
		script, err := txscript.SignTxOutput(chainParams, tx, i,
			...
			sigScript, fnoec.STEcdsaSecp256k1)
		if err != nil {
			return err
		}
