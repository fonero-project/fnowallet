...
func (w *Wallet) LiveTicketHashes(chainClient *fnorpcclient.Client, includeImmature bool) ([]chainhash.Hash, error) {
	...
	// these transactions are not part of the wallet's transaction history, fnod
	// must be queried for their blockchain height.  This functionality requires
	// the fnod transaction index to be enabled.
...
// pruned tickets.
func (w *Wallet) TicketHashesForVotingAddress(votingAddr fnoutil.Address) ([]chainhash.Hash, error) {
...
func (w *Wallet) AddTicket(ticket *wire.MsgTx) error {
	...
	err = walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		err := w.StakeMgr.InsertSStx(stakemgrNs, fnoutil.NewTx(ticket))
		if err != nil {
			return err
		}
...
// revocations.
func (w *Wallet) RevokeTickets(chainClient *fnorpcclient.Client) error {
