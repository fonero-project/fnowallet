...
func (w *Wallet) nextAddress(op errors.Op, persist persistReturnedChildFunc, account, branch uint32,
	callOpts ...NextAddressCallOption) (fnoutil.Address, error) {
...
func (w *Wallet) watchFutureAddresses(dbtx walletdb.ReadTx) error {
	...
	for account, a := range w.addressBuffers {
		...
		addrs := make([]fnoutil.Address, 0, totalAddrs)
...
func (w *Wallet) NewExternalAddress(account uint32, callOpts ...NextAddressCallOption) (fnoutil.Address, error) {
...
func (w *Wallet) NewInternalAddress(account uint32, callOpts ...NextAddressCallOption) (fnoutil.Address, error) {
	const op errors.Op = "wallet.NewExternalAddress"
	return w.nextAddress(op, w.persistReturnedChild(nil), account, udb.InternalBranch, callOpts...)
}

func (w *Wallet) newChangeAddress(op errors.Op, persist persistReturnedChildFunc, account uint32) (fnoutil.Address, error) {
...
// policy.
func (w *Wallet) NewChangeAddress(account uint32) (fnoutil.Address, error) {
...
func (w *Wallet) AccountBranchAddressRange(account, branch, start, end uint32) ([]fnoutil.Address, error) {
...
func deriveChildAddresses(key *hdkeychain.ExtendedKey, startIndex, count uint32, params *chaincfg.Params) ([]fnoutil.Address, error) {
	addresses := make([]fnoutil.Address, 0, count)
...
func deriveChildAddress(key *hdkeychain.ExtendedKey, child uint32, params *chaincfg.Params) (fnoutil.Address, error) {
...
func appendChildAddrsRange(addrs *[]fnoutil.Address, key *hdkeychain.ExtendedKey,
