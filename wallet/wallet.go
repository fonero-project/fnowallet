...
import (
	...
	"github.com/fonero-project/fnowallet/deployments"
	"github.com/fonero-project/fnowallet/errors"
	"github.com/fonero-project/fnowallet/wallet/walletdb"
	"github.com/fonero-project/fnowallet/wallet/txauthor"
...
type Wallet struct {
	...
	votingEnabled      bool
	balanceToMaintain  fnoutil.Amount
	poolAddress        fnoutil.Address
	poolFees           float64
	...
	relayFee               fnoutil.Amount
	relayFeeMu             sync.Mutex
	ticketFeeIncrementLock sync.Mutex
	ticketFeeIncrement     fnoutil.Amount
	DisallowFree           bool
	AllowHighFees          bool
...
func (w *Wallet) BalanceToMaintain() fnoutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance fnoutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() fnoutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee fnoutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) TicketFeeIncrement() fnoutil.Amount {
...
func (w *Wallet) SetTicketFeeIncrement(fee fnoutil.Amount) {
	w.ticketFeeIncrementLock.Lock()
...
func (w *Wallet) loadActiveAddrs(ctx context.Context, dbtx walletdb.ReadTx, nb NetworkBackend) (uint64, error) {
	...
	loadBranchAddrs := func(branchKey *hdkeychain.ExtendedKey, n uint32, errs chan<- error) {
		...
		for child := uint32(0); child <= n; child += step {
			child := child
			g.Go(func() error {
				addrs := make([]fnoutil.Address, 0, step)
...
func (w *Wallet) LoadActiveDataFilters(ctx context.Context, n NetworkBackend, reload bool) error {
	...
	if reload {
		n.LoadTxFilter(ctx, true, nil, nil)
	}
...
func (w *Wallet) CommittedTickets(tickets []*chainhash.Hash) ([]*chainhash.Hash, []fnoutil.Address, error) {
	const op errors.Op = "wallet.CommittedTickets"
	hashes := make([]*chainhash.Hash, 0, len(tickets))
	addresses := make([]fnoutil.Address, 0, len(tickets))
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for _, v := range tickets {
			...
			var bestAddr fnoutil.Address
			var bestAmount fnoutil.Amount
...
// message or fnod JSON-RPC request, for the blocks in sidechain and saved in
...
type (
	...
	createMultisigTxRequest struct {
		...
		pubkeys   []*fnoutil.AddressSecpPubKey
		nrequired int8
		minconf   int32
		resp      chan createMultisigTxResponse
	}
	purchaseTicketRequest struct {
		minBalance  fnoutil.Amount
		spendLimit  fnoutil.Amount
		minConf     int32
		ticketAddr  fnoutil.Address
		account     uint32
		numTickets  int
		poolAddress fnoutil.Address
		poolFees    float64
		expiry      int32
		txFee       fnoutil.Amount
		ticketFee   fnoutil.Amount
...
func (w *Wallet) Consolidate(inputs int, account uint32,
	address fnoutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount fnoutil.Amount, pubkeys []*fnoutil.AddressSecpPubKey, nrequired int8, minconf int32) (*CreatedTx, fnoutil.Address, []byte, error) {
...
// tickets.
func (w *Wallet) PurchaseTickets(minBalance, spendLimit fnoutil.Amount, minConf int32, ticketAddr fnoutil.Address, account uint32, numTickets int, poolAddress fnoutil.Address,
	poolFees float64, expiry int32, txFee fnoutil.Amount, ticketFee fnoutil.Amount) ([]*chainhash.Hash, error) {
...
// spending to it in the blockchain or fnod mempool), the next chained address
// is returned.
func (w *Wallet) CurrentAddress(account uint32) (fnoutil.Address, error) {
...
func (w *Wallet) PubKeyForAddress(a fnoutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr fnoutil.Address) (sig []byte, err error) {
...
func VerifyMessage(msg string, addr fnoutil.Address, sig []byte) (bool, error) {
	...
	recoveredAddr, err := fnoutil.NewAddressSecpPubKey(serializedPK, addr.Net())
	if err != nil {
		return false, errors.E(op, err)
	}
...
func (w *Wallet) HaveAddress(a fnoutil.Address) (bool, error) {
...
func (w *Wallet) AccountOfAddress(a fnoutil.Address) (uint32, error) {
...
func (w *Wallet) AddressInfo(a fnoutil.Address) (udb.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *udb.TxDetails, addrMgr *udb.Manager, syncHeight int32, net *chaincfg.Params) (sends, receives []fnojson.ListTransactionsResult) {
	addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
	...
	txTypeStr := fnojson.LTTTRegular
	switch details.TxType {
	case stake.TxTypeSStx:
		txTypeStr = fnojson.LTTTTicket
	case stake.TxTypeSSGen:
		txTypeStr = fnojson.LTTTVote
	case stake.TxTypeSSRtx:
		txTypeStr = fnojson.LTTTRevocation
	}

	// Fee can only be determined if every input is a debit.
	var feeF64 float64
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal fnoutil.Amount
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := fnoutil.Amount(output.Value).ToCoin()
		result := fnojson.ListTransactionsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   BlockIndex
			//
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]fnojson.ListTransactionsResult, error) {
	const op errors.Op = "wallet.ListSinceBlock"
	txList := []fnojson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]fnojson.ListTransactionsResult, error) {
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]fnojson.ListTransactionsResult, error) {
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*fnoutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]fnojson.ListTransactionsResult, error) {
...
// transaction.
func (w *Wallet) ListTransactionDetails(txHash *chainhash.Hash) ([]fnojson.ListTransactionsResult, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		txList = make([]fnojson.ListTransactionsResult, 0, len(sends)+len(receives))
		txList = append(txList, receives...)
		txList = append(txList, sends...)
		return nil
	})
...
func (w *Wallet) GetTicketInfoPrecise(chainClient *fnorpcclient.Client, hash *chainhash.Hash) (*TicketSummary, *wire.BlockHeader, error) {
...
func (w *Wallet) GetTicketsPrecise(f func([]*TicketSummary, *wire.BlockHeader) (bool, error), chainClient *fnorpcclient.Client, startBlock, endBlock *BlockIdentifier) error {
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*fnoutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32, addresses map[string]struct{}) ([]*fnojson.ListUnspentResult, error) {
	const op errors.Op = "wallet.ListUnspent"
	var results []*fnojson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		scSwitch:
			...
			result := &fnojson.ListUnspentResult{
...
func (w *Wallet) DumpWIFPrivateKey(addr fnoutil.Address) (string, error) {
	...
	wif, err := fnoutil.NewWIF(privKey, w.chainParams, fnoec.SignatureType(privKey.GetType()))
	if err != nil {
		return "", errors.E(op, err)
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *fnoutil.WIF) (string, error) {
	...
	if n, err := w.NetworkBackend(); err == nil {
		err := n.LoadTxFilter(context.TODO(), false, []fnoutil.Address{addr}, nil)
		if err != nil {
			return "", errors.E(op, err)
		}
	}
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr fnoutil.Address) ([]byte, error) {
...
type StakeInfoData struct {
	BlockHeight  int64
	TotalSubsidy fnoutil.Amount
	Sdiff        fnoutil.Amount
		...
		for it.Next() {
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spender):
					...
					// earned by fees.
					res.TotalSubsidy += fnoutil.Amount(spender.TxIn[0].ValueIn)
...
func (w *Wallet) StakeInfoPrecise(chainClient *fnorpcclient.Client) (*StakeInfoData, error) {
	const op errors.Op = "wallet.StakeInfoPrecise"
	// This is only needed for the total count and can be optimized.
	mempoolTicketsFuture := chainClient.GetRawMempoolAsync(fnojson.GRMTickets)
...
func (w *Wallet) LockedOutpoints() []fnojson.TransactionInput {
	locked := make([]fnojson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = fnojson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr fnoutil.Address) error {
...
func ticketMatured(params *chaincfg.Params, txHeight, curHeight int32) bool {
	// fnod has an off-by-one in the calculation of the ticket
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType, additionalPrevScripts map[wire.OutPoint][]byte,
	additionalKeysByAddress map[string]*fnoutil.WIF, p2shRedeemScriptsByAddress map[string][]byte) ([]SignatureError, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr fnoutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr fnoutil.Address) ([]byte, error) {
...
// address.
func (w *Wallet) CreateSignature(tx *wire.MsgTx, idx uint32, addr fnoutil.Address, hashType txscript.SigHashType, prevPkScript []byte) (sig, pubkey []byte, err error) {
...
func Open(cfg *Config) (*Wallet, error) {
	...
	// Amounts
	w.ticketFeeIncrement, err = fnoutil.NewAmount(cfg.TicketFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
	w.relayFee, err = fnoutil.NewAmount(cfg.RelayFee)
	if err != nil {
		return nil, errors.E(op, err)
	}
