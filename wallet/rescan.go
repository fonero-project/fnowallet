...
func NewRescanFilter(addresses []fnoutil.Address, unspentOutPoints []*wire.OutPoint) *RescanFilter {
	filter := &RescanFilter{
		pubKeyHashes:        map[[ripemd160.Size]byte]struct{}{},
		scriptHashes:        map[[ripemd160.Size]byte]struct{}{},
		compressedPubKeys:   map[[33]byte]struct{}{},
...
func (f *RescanFilter) AddAddress(a fnoutil.Address) {
	switch a := a.(type) {
...
func (f *RescanFilter) ExistsAddress(a fnoutil.Address) (ok bool) {
	switch a := a.(type) {
	case *fnoutil.AddressPubKeyHash:
...
func (f *RescanFilter) RemoveAddress(a fnoutil.Address) {
	switch a := a.(type) {
	case *fnoutil.AddressPubKeyHash:
