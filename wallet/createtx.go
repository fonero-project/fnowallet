...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb fnoutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(fnoutil.Amount) (*txauthor.InputDetail, error) {
				inputDetail, err := sourceImpl.SelectInputs(fnoutil.MaxAmount)
...
func (s *secretSource) GetKey(addr fnoutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr fnoutil.Address) ([]byte, error) {
...
func (w *Wallet) checkHighFees(totalInput fnoutil.Amount, tx *wire.MsgTx) error {
	if w.AllowHighFees {
		return nil
	}
...
//
// Fonero: This func also sends the transaction, and if successful, inserts it
...
func (w *Wallet) txToOutputsInternal(op errors.Op, outputs []*wire.TxOut, account uint32, minconf int32,
	n NetworkBackend, randomizeChangeIdx bool, txFee fnoutil.Amount) (*txauthor.AuthoredTx, error) {
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := fnoutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisig(op errors.Op, account uint32, amount fnoutil.Amount, pubkeys []*fnoutil.AddressSecpPubKey,
	nRequired int8, minconf int32) (*CreatedTx, fnoutil.Address, []byte, error) {

	var (
		ctx      *CreatedTx
		addr     fnoutil.Address
		msScript []byte
	)
	err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		var err error
...
func (w *Wallet) txToMultisigInternal(op errors.Op, dbtx walletdb.ReadWriteTx, account uint32, amount fnoutil.Amount,
	pubkeys []*fnoutil.AddressSecpPubKey, nRequired int8, minconf int32) (*CreatedTx, fnoutil.Address, []byte, error) {

	addrmgrNs := dbtx.ReadWriteBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadWriteBucket(wtxmgrNamespaceKey)

	txToMultisigError := func(err error) (*CreatedTx, fnoutil.Address, []byte, error) {
	...
	// fee estimator.
	var feeEstForTx fnoutil.Amount
	...
	scAddr, err := fnoutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
	...
	// Request updates from fnod for new transactions sent to this
	// script hash address.
	err = n.LoadTxFilter(context.TODO(), false, []fnoutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(errors.E(op, err))
	}
...
func (w *Wallet) compressWallet(op errors.Op, maxNumIns int, account uint32, changeAddr fnoutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(op errors.Op, dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr fnoutil.Address) (*chainhash.Hash, error) {
...
// passed.
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint, input *extendedOutPoint, addrVote fnoutil.Address,
	addrSubsidy fnoutil.Address, ticketCost int64, addrPool fnoutil.Address) (*wire.MsgTx, error) {
	...
	zeroed := [20]byte{}
	addrZeroed, err := fnoutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, err = txscript.GenerateSStxAddrPush(addrPool,
			fnoutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addrSubsidy,
		fnoutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
		return nil, errors.E(errors.Op("txscript.GenerateSStxAddrPush"), errors.Invalid,
...
func (w *Wallet) purchaseTickets(op errors.Op, req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(errors.Op, persistReturnedChildFunc, uint32) (fnoutil.Address, error) {
			return addr, err
		}
	}
	...
	case *fnoutil.AddressScriptHash:
		stakeSubmissionPkScriptSize = txsizes.P2SHPkScriptSize + 1
	case *fnoutil.AddressPubKeyHash, nil:
	...
	var neededPerTicket, ticketFee fnoutil.Amount
	var estSize int
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * fnoutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy fnoutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       fnoutil.Amount(eopPool.amt),
		...
		err = w.checkHighFees(fnoutil.Amount(eop.amt), ticket)
		if err != nil {
			return ticketHashes, errors.E(op, err)
		}
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount fnoutil.Amount, currentHeight int32) ([]udb.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)
	var outTotal fnoutil.Amount
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr fnoutil.Address) (chainec.PrivateKey, bool, error) {
		...
		return key, true, nil // secp256k1 pubkeys are always compressed in Fonero
	}
	var getScript txscript.ScriptClosure = func(addr fnoutil.Address) ([]byte, error) {
	...
	signedScript, err := txscript.SignTxOutput(w.chainParams, tx, inputToSign,
		...
		tx.TxIn[inputToSign].SignatureScript, fnoec.STEcdsaSecp256k1)
	if err != nil {
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB fnoutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if fnoutil.Amount(output.Value) > feeEstimate {
			amount := fnoutil.Amount(output.Value) - feeEstimate
