...
const DefaultRelayFeePerKb fnoutil.Amount = 1e4

// IsDustAmount determines whether a transaction output value and script length would
// cause the output to be considered dust.  Transactions with dust outputs are
// not standard and are rejected by mempools with default policies.
func IsDustAmount(amount fnoutil.Amount, scriptSize int, relayFeePerKb fnoutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb fnoutil.Amount) bool {
	...
	return IsDustAmount(fnoutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
func CheckOutput(output *wire.TxOut, relayFeePerKb fnoutil.Amount) error {
	if output.Value < 0 {
		return errors.E(errors.Invalid, "transaction output amount is negative")
	}
	if output.Value > fnoutil.MaxAmount {
...
func FeeForSerializeSize(relayFeePerKb fnoutil.Amount, txSerializeSize int) fnoutil.Amount {
	fee := relayFeePerKb * fnoutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > fnoutil.MaxAmount {
		fee = fnoutil.MaxAmount
	}
...
func PaysHighFees(totalInput fnoutil.Amount, tx *wire.MsgTx) bool {
