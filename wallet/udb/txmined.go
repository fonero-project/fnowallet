...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, fnoutil.Address) (uint32, error)
}
...
// BlockCFilter is a compact filter for a Fonero block.
...
func stakeValidate(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for i := range blockRec.transactions {
		...
		for i, output := range txRec.MsgTx.TxOut {
			...
			minedBalance += fnoutil.Amount(output.Value)
		}
...
func (s *Store) AddCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     fnoutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if block == nil {
		...
		v := valueUnminedCredit(fnoutil.Amount(rec.MsgTx.TxOut[index].Value),
	...
	txOutAmt := fnoutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) AddMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32) error {
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		fnoutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func approvesParent(voteBits uint16) bool {
	return fnoutil.IsFlagSet16(voteBits, fnoutil.BlockValid)
}
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket, addr fnoutil.Address) ([]*MultisigCredit, error) {
...
// without error.
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket, needed fnoutil.Amount, syncHeight int32, minConf int32, all bool, account uint32) ([]*Credit, error) {
	var eligible []*minimalCredit
	var toUse []*minimalCredit
	var unspent []*Credit
	found := fnoutil.Amount(0)
...
type InputSource struct {
	source func(fnoutil.Amount) (*txauthor.InputDetail, error)
}
...
func (s *InputSource) SelectInputs(target fnoutil.Amount) (*txauthor.InputDetail, error) {
...
func (s *Store) MakeInputSource(ns, addrmgrNs walletdb.ReadBucket, account uint32, minConf, syncHeight int32) InputSource {
	...
	var (
		currentTotal      fnoutil.Amount
		currentInputs     []*wire.TxIn
		currentScripts    [][]byte
		redeemScriptSizes []int
	)

	f := func(target fnoutil.Amount) (*txauthor.InputDetail, error) {
...
func (s *Store) balanceFullScan(ns, addrmgrNs walletdb.ReadBucket, minConf int32, syncHeight int32) (map[uint32]*Balances, error) {
	...
	for k, v := c.First(); k != nil; k, v = c.Next() {
		...
		case txscript.OP_SSTX:
			...
			votingAuthorityAmt := fnoutil.Amount(0)
			lockedByTicketsAmt := fnoutil.Amount(0)
			...
			totalInputAmount := fnoutil.Amount(0)
			...
			for _, txin := range rec.MsgTx.TxIn {
				...
				if rawUnmined != nil {
					...
					if int(txin.PreviousOutPoint.Index) < len(tx.TxOut) {
						totalInputAmount += fnoutil.Amount(tx.TxOut[txin.PreviousOutPoint.Index].Value)
					}
				} else {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards fnoutil.Amount
	ImmatureStakeGeneration fnoutil.Amount
	LockedByTickets         fnoutil.Amount
	Spendable               fnoutil.Amount
	Total                   fnoutil.Amount
	VotingAuthority         fnoutil.Amount
	Unconfirmed             fnoutil.Amount
}
...
func (s *Store) TotalInput(dbtx walletdb.ReadTx, tx *wire.MsgTx) (fnoutil.Amount, error) {
	ns := dbtx.ReadBucket(wtxmgrBucketKey)

	var total fnoutil.Amount
	for _, in := range tx.TxIn {
		...
		total += fnoutil.Amount(tx.TxOut[in.PreviousOutPoint.Index].Value)
	}
