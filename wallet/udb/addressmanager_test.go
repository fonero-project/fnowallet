...
func testManagedPubKeyAddress(tc *testContext, prefix string, gotAddr ManagedPubKeyAddress, wantAddr *expectedAddr) bool {
	...
	case tc.watchingOnly:
		...
		if !checkManagerError(tc.t, testName, err, apperrors.ErrWatchingOnly) {
			return false
		}
	case tc.unlocked:
		if err != nil {
	...
	default:
		...
		testName := fmt.Sprintf("%s PrivKey", prefix)
		if !checkManagerError(tc.t, testName, err, apperrors.ErrLocked) {
			return false
		}
	}
...
func testExternalAddresses(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i := 0; i < len(expectedExternalAddrs); i++ {
			...
			utilAddr, err := fnoutil.NewAddressPubKeyHash(pkHash,
...
func testLocking(tc *testContext) bool {
	...
	// address manager.
	err := tc.manager.Lock()
	wantErrCode := apperrors.ErrLocked
	if tc.watchingOnly {
		wantErrCode = apperrors.ErrWatchingOnly
	}
	...
	if tc.watchingOnly {
		if !checkManagerError(tc.t, "Unlock", err, apperrors.ErrWatchingOnly) {
			return false
		}
	} else if err != nil {
		tc.t.Errorf("Unlock: unexpected error: %v", err)
	...
	err = tc.manager.Unlock(privPassphrase)
	if tc.watchingOnly {
		if !checkManagerError(tc.t, "Unlock2", err, apperrors.ErrWatchingOnly) {
			return false
		}
	} else if err != nil {
		tc.t.Errorf("Unlock: unexpected error: %v", err)
	...
	wantErrCode = apperrors.ErrWrongPassphrase
	if tc.watchingOnly {
		wantErrCode = apperrors.ErrWatchingOnly
	}
...
func testImportPrivateKey(tc *testContext) bool {
	...
	if tc.create {
		for i, test := range tests {
			...
			wif, err := fnoutil.DecodeWIF(test.in)
			if err != nil {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			utilAddr, err := fnoutil.NewAddressPubKeyHash(
...
func testImportScript(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			// Use the Address API to retrieve each of the expected
			// new addresses and ensure they're accurate.
			utilAddr, err := fnoutil.NewAddressScriptHash(test.in,
				chainParams)
			if err != nil {
...
func testMarkUsed(tc *testContext) bool {
	...
	for i, test := range tests {
		...
		case addrPubKeyHash:
			...
			addr, err = fnoutil.NewAddressPubKeyHash(addrHash, chainParams, chainec.ECTypeSecp256k1)
		case addrScriptHash:
			testtype = "addrScriptHash"
			addr, err = fnoutil.NewAddressScriptHashFromHash(addrHash, chainParams)
		default:
			panic("unreachable")
		}
		if err != nil {
...
func testChangePassphrase(tc *testContext) bool {
	...
	if !checkManagerError(tc.t, testName, err, apperrors.ErrCrypto) {
		return false
	}
	...
	if !checkManagerError(tc.t, testName, err, apperrors.ErrWrongPassphrase) {
		return false
	}
...
func testNewAccount(tc *testContext) bool {
	...
	wantErrCode := apperrors.ErrDuplicateAccount
	if !checkManagerError(tc.t, testName, err, wantErrCode) {
		return false
	}
	...
	_, err = tc.manager.NewAccount(testName)
	wantErrCode = apperrors.ErrInvalidAccount
	if !checkManagerError(tc.t, testName, err, wantErrCode) {
		return false
	}
...
func testLookupAccount(tc *testContext) bool {
	...
	wantErrCode := apperrors.ErrAccountNotFound
	if !checkManagerError(tc.t, testName, err, wantErrCode) {
		return false
	}
	...
	for i, addr := range expectedAddrs {
		addr, err := fnoutil.NewAddressPubKeyHash(addr.addressHash,
...
func testWatchingOnly(tc *testContext) bool {
	...
	mgr, err := Open(namespace, pubPassphrase,
	...
	mgr, err = Open(namespace, pubPassphrase, &chaincfg.TestNetParams,
		nil)
	if err != nil {
...
func TestManager(t *testing.T) {
	...
	err = Create(mgrNamespace, seed, pubPassphrase,
		privPassphrase, &chaincfg.TestNetParams, fastScrypt, false)
	if err != nil {
		t.Errorf("Create: unexpected error: %v", err)
		return
	}
	mgr, err := Open(mgrNamespace, pubPassphrase,
		&chaincfg.TestNetParams, nil)
	if err != nil {
	...
	err = Create(mgrNamespace, seed, pubPassphrase,
		privPassphrase, &chaincfg.TestNetParams, fastScrypt, false)
	if !checkManagerError(t, "Create existing", err, apperrors.ErrAlreadyExists) {
		mgr.Close()
		return
	}
	...
	*TstLatestMgrVersion++
	_, err = Open(mgrNamespace, pubPassphrase,
		&chaincfg.TestNetParams, nil)
	if !checkManagerError(t, "Upgrade needed", err, apperrors.ErrUpgrade) {
		return
	}
	*TstLatestMgrVersion--
...
func TestEncryptDecryptErrors(t *testing.T) {
	...
	checkManagerError(t, "failed encryption", err, apperrors.ErrCrypto)

	// Make sure to cover the ErrCrypto error path in Decrypt.
	TstRunWithFailingCryptoKeyPriv(mgr, func() {
		_, err = mgr.Decrypt(CKTPrivate, []byte{})
	})
	checkManagerError(t, "failed decryption", err, apperrors.ErrCrypto)
}
