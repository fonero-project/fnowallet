...
import (
	...
	"github.com/fonero-project/fnowallet/wallet/internal/snacl"
...
func normalizeAddress(addr fnoutil.Address) fnoutil.Address {
...
// now created using the coin types assigned to Fonero in SLIP0044:
//
...
// to the coin type registered to Fonero as per SLIP0044.  On mainnet, this
...
//
// This function MUST be called with the manager lock held for writes.
func (m *Manager) loadAddress(ns walletdb.ReadBucket, address fnoutil.Address) (ManagedAddress, error) {
...
func (m *Manager) Address(ns walletdb.ReadBucket, address fnoutil.Address) (ManagedAddress, error) {
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket, address fnoutil.Address) (uint32, error) {
...
func (m *Manager) ImportPrivateKey(ns walletdb.ReadWriteBucket, wif *fnoutil.WIF) (ManagedPubKeyAddress, error) {
	defer m.mtx.Unlock()
	m.mtx.Lock()
	...
	pubKeyHash := fnoutil.Hash160(serializedPubKey)
...
func (m *Manager) ImportScript(ns walletdb.ReadWriteBucket, script []byte) (ManagedScriptAddress, error) {
	...
	// Prevent duplicates.
	scriptHash := fnoutil.Hash160(script)
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address fnoutil.Address) error {
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr fnoutil.Address) error) error {
	m.mtx.Lock()
	defer m.mtx.Unlock()
...
func (m *Manager) PrivateKey(ns walletdb.ReadBucket, addr fnoutil.Address) (key chainec.PrivateKey, done func(), err error) {
...
func (m *Manager) RedeemScript(ns walletdb.ReadBucket, addr fnoutil.Address) (script []byte, done func(), err error) {
	// Lock the manager mutex for writes.  This protects read access to m.locked
