...
func (s *StakeStore) insertSStx(ns walletdb.ReadWriteBucket, sstx *fnoutil.Tx) error {
...
func (s *StakeStore) dumpSStxHashesForAddress(ns walletdb.ReadBucket, addr fnoutil.Address) ([]chainhash.Hash, error) {
	// Extract the HASH160 script hash; if it's not 20 bytes
	// long, return an error.
	hash160 := addr.ScriptAddress()
	_, addrIsP2SH := addr.(*fnoutil.AddressScriptHash)
...
func (s *StakeStore) sstxAddress(ns walletdb.ReadBucket, hash *chainhash.Hash) (fnoutil.Address, error) {
	// Access the database and store the result locally.
	...
	if p2sh {
		addr, err = fnoutil.NewAddressScriptHashFromHash(thisHash160, s.Params)
	} else {
		addr, err = fnoutil.NewAddressPubKeyHash(thisHash160, s.Params, fnoec.STEcdsaSecp256k1)
	}
	if err != nil {
		return nil, err
	}
...
func (s *StakeStore) updateStakePoolUserTickets(ns walletdb.ReadWriteBucket, user fnoutil.Address, ticket *PoolTicket) error {
	_, isScriptHash := user.(*fnoutil.AddressScriptHash)
	_, isP2PKH := user.(*fnoutil.AddressPubKeyHash)
...
func (s *StakeStore) removeStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user fnoutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*fnoutil.AddressScriptHash)
	_, isP2PKH := user.(*fnoutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		return errors.E(errors.Invalid, errors.Errorf("voting address type %T", user))
...
// database, it adds it.
func (s *StakeStore) updateStakePoolUserInvalTickets(ns walletdb.ReadWriteBucket, user fnoutil.Address, ticket *chainhash.Hash) error {
	_, isScriptHash := user.(*fnoutil.AddressScriptHash)
	_, isP2PKH := user.(*fnoutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
		return errors.E(errors.Invalid, errors.Errorf("voting address type %T", user))
...
func stakePoolUserInfo(ns walletdb.ReadBucket, user fnoutil.Address) (*StakePoolUser, error) {
	_, isScriptHash := user.(*fnoutil.AddressScriptHash)
	_, isP2PKH := user.(*fnoutil.AddressPubKeyHash)
	if !(isScriptHash || isP2PKH) {
...
func (s *StakeStore) StakePoolUserInfo(ns walletdb.ReadBucket, user fnoutil.Address) (*StakePoolUser, error) {
