...
func deserializeSStxRecord(serializedSStxRecord []byte, dbVersion uint32) (*sstxRecord, error) {
	...
	case dbVersion < 3:
		...
		// Create and save the fnoutil.Tx of the read MsgTx and set its index.
		tx := fnoutil.NewTx(msgTx)
		tx.SetIndex(fnoutil.TxIndexUnknown)
	...
	case dbVersion >= 3:
		...
		return &sstxRecord{tx: fnoutil.NewTx(&tx), ts: time.Unix(unixTime, 0)}, nil
