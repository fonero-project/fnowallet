...
type ManagedAddress interface {
	...
	// Address returns a fnoutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *fnoutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the fnoutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() fnoutil.Address {
	return a.address
}
...
func newManagedAddressWithoutPrivKey(m *Manager, account uint32, pubKey chainec.PublicKey, compressed bool) (*managedAddress, error) {
	...
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = fnoutil.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = fnoutil.Hash160(pubKey.SerializeUncompressed())
	}
	address, err := fnoutil.NewAddressPubKeyHash(pubKeyHash, m.chainParams,
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *fnoutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() fnoutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := fnoutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
