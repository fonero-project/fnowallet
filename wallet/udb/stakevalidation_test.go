...
func TestStakeInvalidationOfTip(t *testing.T) {
	...
	block1Header := g.generate(fnoutil.BlockValid)
	block2Header := g.generate(fnoutil.BlockValid)
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		if bal.Total != fnoutil.Amount(block2Tx.TxOut[0].Value) {
			t.Errorf("Wrong balance: expected %v got %v", fnoutil.Amount(block2Tx.TxOut[0].Value), bal)
		}
		...
		if credits[0].Amount != fnoutil.Amount(block2Tx.TxOut[0].Value) {
		...
		*/
		...
		if bal.Total != fnoutil.Amount(block1Tx.TxOut[0].Value) {
			t.Errorf("Wrong balance: expected %v got %v", fnoutil.Amount(block1Tx.TxOut[0].Value), bal)
		}
		credits, err = s.UnspentOutputs(ns)
		if err != nil {
			return err
		...
		if credits[0].Amount != fnoutil.Amount(block1Tx.TxOut[0].Value) {
