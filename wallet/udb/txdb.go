...
func fetchMinedBalance(ns walletdb.ReadBucket) (fnoutil.Amount, error) {
	v := ns.Get(rootMinedBalance)
	if len(v) != 8 {
		return 0, errors.E(errors.IO, errors.Errorf("mined balance len %d", len(v)))
	}
	return fnoutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt fnoutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (fnoutil.Amount, error) {
	if len(v) < 9 {
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (fnoutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return fnoutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (fnoutil.Amount, bool, error) {
	if len(v) < 9 {
		return 0, false, errors.E(errors.IO, errors.Errorf("credit len %d", len(v)))
	}
	return fnoutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (fnoutil.Amount, error) {
	...
	return fnoutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (fnoutil.Amount, error) {
	...
	return fnoutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = fnoutil.Amount(byteOrder.Uint64(it.cv))
...
func valueDebit(amount fnoutil.Amount, credKey []byte) []byte {
	v := make([]byte, 80)
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount fnoutil.Amount, block *Block, credKey []byte) error {
...
func extractRawDebitAmount(v []byte) fnoutil.Amount {
	return fnoutil.Amount(byteOrder.Uint64(v[:8]))
}
...
func valueUnminedCredit(amount fnoutil.Amount, change bool, opCode uint8,
...
func fetchRawUnminedCreditAmount(v []byte) (fnoutil.Amount, error) {
...
func fetchRawUnminedCreditAmountChange(v []byte) (fnoutil.Amount, bool, error) {
	if len(v) < unconfValueSizeLegacy {
		return 0, false, errors.E(errors.IO, errors.Errorf("unmined credit len %d", len(v)))
	}
	amt := fnoutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
...
func valueMultisigOut(sh [ripemd160.Size]byte, m uint8, n uint8,
	...
	blockHeight uint32, amount fnoutil.Amount, spentBy chainhash.Hash,
...
func fetchMultisigOut(k, v []byte) (*MultisigOut, error) {
	...
	mso.Amount = fnoutil.Amount(byteOrder.Uint64(v[59:67]))
...
func fetchMultisigOutAmount(v []byte) fnoutil.Amount {
	return fnoutil.Amount(byteOrder.Uint64(v[59:67]))
}
