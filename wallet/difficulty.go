...
// This code was copied from fnod/blockchain/difficulty.go and modified for
// fnowallet's header storage.
...
func (w *Wallet) ancestorHeaderAtHeight(dbtx walletdb.ReadTx, h *wire.BlockHeader, chain []*BlockNode, height int32) (*wire.BlockHeader, error) {
	...
	case height > int32(h.Height), height < 0:
		return nil, nil // fnod's blockNode.Ancestor returns nil for child heights
	}
...
// in DCP0001.
func (w *Wallet) nextRequiredDCP0001PoSDifficulty(dbtx walletdb.ReadTx, curHeader *wire.BlockHeader, chain []*BlockNode) (fnoutil.Amount, error) {
	...
	if nextHeight < stakeDiffStartHeight {
		return fnoutil.Amount(w.chainParams.MinimumStakeDiff), nil
	}
	...
	if nextHeight%intervalSize != 0 {
		return fnoutil.Amount(curDiff), nil
	}
	...
	return fnoutil.Amount(sdiff), nil
}
...
// trusted full node.
func (w *Wallet) NextStakeDifficulty() (fnoutil.Amount, error) {
	const op errors.Op = "wallet.NextStakeDifficulty"
	var sdiff fnoutil.Amount
...
func (w *Wallet) NextStakeDifficultyAfterHeader(h *wire.BlockHeader) (fnoutil.Amount, error) {
...
func (w *Wallet) validateHeaderChainDifficulties(dbtx walletdb.ReadTx, chain []*BlockNode, idx int) ([]*BlockNode, error) {
	...
	for ; idx < len(chain); idx++ {
		...
		if deployments.DCP0001.Active(int32(h.Height), w.chainParams) {
			...
			if fnoutil.Amount(h.SBits) != sdiff {
				err := errors.Errorf("%v has invalid PoS difficulty, got %v, want %v",
					&hash, fnoutil.Amount(h.SBits), sdiff)
				return chain[idx:], errors.E(op, errors.Consensus, err)
			}
		}
