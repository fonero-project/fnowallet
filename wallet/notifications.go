...
func lookupOutputChain(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails,
	cred udb.CreditRecord) (account uint32, internal bool, address fnoutil.Address,
...
func makeTxSummary(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails) TransactionSummary {
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		for _, txOut := range details.MsgTx.TxOut {
			fee -= fnoutil.Amount(txOut.Value)
		}
	}
...
func makeTicketSummary(chainClient *fnorpcclient.Client, dbtx walletdb.ReadTx, w *Wallet, details *udb.TicketDetails) *TicketSummary {
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]fnoutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]fnoutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]fnoutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
...
func (s *NotificationServer) notifyUnminedTransaction(dbtx walletdb.ReadTx, details *udb.TxDetails) {
	...
	bals := make(map[uint32]fnoutil.Amount)
