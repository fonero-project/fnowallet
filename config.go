...
import (
	...
	"github.com/fonero-project/fnowallet/version"
...
const (
	defaultCAFilename          = "fnod.cert"
	defaultConfigFilename      = "fnowallet.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "fnowallet.log"
	...
	defaultMaxFee                    fnoutil.Amount = 1e6
	defaultMinFee                    fnoutil.Amount = 1e5
...
var (
	fnodDefaultCAFile  = filepath.Join(fnoutil.AppDataDir("fnod", false), "rpc.cert")
	defaultAppDataDir  = fnoutil.AppDataDir("fnowallet", false)
...
type config struct {
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of fnod RPC server to connect to"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with fnod"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	DcrdUsername     string                  `long:"fnodusername" description:"Username for fnod authentication"`
	DcrdPassword     string                  `long:"fnodpassword" default-mask:"-" description:"Password for fnod authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy JSON-RPC and fnod authentication (if fnodusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy JSON-RPC and fnod authentication (if fnodpassword is unset)"`
...
//
// The above results in fnowallet functioning properly without any config
...
func loadConfig(ctx context.Context) (*config, []string, error) {
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local fnod cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local fnod and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					fnodCertExists, err := cfgutil.FileExists(
						fnodDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return loadConfigError(err)
					...
					if fnodCertExists {
						cfg.CAFile.Value = fnodDefaultCAFile
					}
				}
			}
		}
	...
	// If the fnod username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for fnod and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for fnod and wallet.
	if cfg.DcrdUsername == "" {
