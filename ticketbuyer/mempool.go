...
func (t *TicketPurchaser) ownTicketsInMempool() (int, error) {
	...
	if t.votingAddress != nil {
		tiHashes, err := t.fnodChainSvr.GetRawMempool(fnojson.GRMTickets)
		if err != nil {
			return 0, err
		}
		...
		for i := range tiHashes {
			raw, err := t.fnodChainSvr.GetRawTransactionVerbose(tiHashes[i])
			if err != nil {
				return 0, err
			}
...
func (t *TicketPurchaser) allTicketsInMempool() (int, error) {
	tfi, err := t.fnodChainSvr.TicketFeeInfo(&zeroUint32, &zeroUint32)
	if err != nil {
		return 0, err
	}
