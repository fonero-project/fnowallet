...
type TicketPurchaser struct {
	...
	fnodChainSvr     *fnorpcclient.Client
...
func (t *TicketPurchaser) BalanceToMaintain() fnoutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetBalanceToMaintain(balanceToMaintain int64) {
	t.purchaserMtx.Lock()
	t.balanceToMaintain = fnoutil.Amount(balanceToMaintain)
	t.purchaserMtx.Unlock()
}

// MaxPriceAbsolute returns the max absolute price to purchase a ticket.
func (t *TicketPurchaser) MaxPriceAbsolute() fnoutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetMaxPriceAbsolute(maxPriceAbsolute int64) {
	t.purchaserMtx.Lock()
	t.maxPriceAbsolute = fnoutil.Amount(maxPriceAbsolute)
	t.purchaserMtx.Unlock()
}
...
func (t *TicketPurchaser) MaxFee() fnoutil.Amount {
	t.purchaserMtx.Lock()
	maxFee := t.maxFee
	t.purchaserMtx.Unlock()
	return maxFee
...
func (t *TicketPurchaser) SetMaxFee(maxFee int64) {
	t.purchaserMtx.Lock()
	t.maxFee = fnoutil.Amount(maxFee)
	t.purchaserMtx.Unlock()
}

// MinFee returns the min ticket fee per KB to use when purchasing tickets.
func (t *TicketPurchaser) MinFee() fnoutil.Amount {
	t.purchaserMtx.Lock()
	minFee := t.minFee
	t.purchaserMtx.Unlock()
	return minFee
...
func (t *TicketPurchaser) SetMinFee(minFee int64) {
	t.purchaserMtx.Lock()
	t.minFee = fnoutil.Amount(minFee)
	t.purchaserMtx.Unlock()
}

// VotingAddress returns the address to send ticket outputs to.
func (t *TicketPurchaser) VotingAddress() fnoutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetVotingAddress(votingAddress fnoutil.Address) {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) PoolAddress() fnoutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetPoolAddress(poolAddress fnoutil.Address) {
	t.purchaserMtx.Lock()
...
func NewTicketPurchaser(cfg *Config,
	...
	return &TicketPurchaser{
		...
		account:           account,
		balanceToMaintain: fnoutil.Amount(cfg.BalanceToMaintainAbsolute),
		maxFee:            fnoutil.Amount(cfg.MaxFee),
		minFee:            fnoutil.Amount(cfg.MinFee),
		maxPerBlock:       cfg.MaxPerBlock,
		maxPriceAbsolute:  fnoutil.Amount(cfg.MaxPriceAbsolute),
...
type PurchaseStats struct {
	...
	PriceAverage  fnoutil.Amount
	PriceNext     fnoutil.Amount
	PriceCurrent  fnoutil.Amount
	Purchased     int
	LeftWindow    int
	Balance       fnoutil.Amount
	TicketPrice   fnoutil.Amount
}
...
func (t *TicketPurchaser) Purchase(height int64) (*PurchaseStats, error) {
	...
	if t.firstStart {
		...
		log.Debugf("Transaction relay fee: %v", fnoutil.Amount(t.cfg.TxFee))
	...
	if refreshStakeInfo {
		...
		for i := 1; i <= stakeInfoReqTries; i++ {
			curStakeInfo, err = t.wallet.StakeInfoPrecise(t.fnodChainSvr)
			if err != nil {
		...
		t.stakeVoteSubsidy = fnoutil.Amount(subsidy)
	...
	if avgPriceAmt < fnoutil.Amount(t.activeNet.MinimumStakeDiff) {
		avgPriceAmt = fnoutil.Amount(t.activeNet.MinimumStakeDiff)
	}
	...
	if err != nil && t.fnodChainSvr != nil {
		// Wallet failed to calculate sdiff (DCP0001 may not be active), so
		// query it over RPC.
		var sd *fnojson.GetStakeDifficultyResult
		sd, err = t.fnodChainSvr.GetStakeDifficulty()
		if err == nil {
			nextStakeDiff, err = fnoutil.NewAmount(sd.NextStakeDifficulty)
			if err != nil {
				return ps, err
			}
		}
	...
	sDiffEsts, err := t.fnodChainSvr.EstimateStakeDiff(nil)
	if err == nil {
		ps.PriceNext, err = fnoutil.NewAmount(sDiffEsts.Expected)
		if err != nil {
			return ps, err
		}
	...
	var maxPriceAmt fnoutil.Amount
	...
	oneBlock := uint32(1)
	info, err := t.fnodChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	...
	if !t.cfg.NoSpreadTicketPurchases && toBuyForBlock > 0 {
		...
		log.Debugf("Your average purchase price for tickets in the pool is %.2f FNO", yourAvgTixPrice)
		log.Debugf("Available funds of %.2f FNO can buy %.2f tickets, %.2f tickets per block",
		...
		log.Debugf("Redeemed ticket value expected is %.2f FNO, buys %.2f tickets, %.2f%% more",
			redeemedFunds, tixToBuyWithRedeemedFunds, tixToBuyWithRedeemedFunds/tixCanBuy*100)
		log.Debugf("Stake reward expected is %.2f FNO, buys %.2f tickets, %.2f%% more",
	...
	// have enough moneys.
	notEnough := func(bal fnoutil.Amount, toBuy int, sd fnoutil.Amount) bool {
	...
	// Purchase tickets.
	poolFeesAmt, err := fnoutil.NewAmount(t.cfg.PoolFees)
	if err != nil {
		return ps, err
	}
