...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64) (fnoutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt fnoutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		poolValue, err := t.fnodChainSvr.GetTicketPoolValue()
		if err != nil {
			return 0, err
		}
		bestBlockH, err := t.fnodChainSvr.GetBestBlockHash()
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.fnodChainSvr.GetBlockHeader(bestBlockH)
		if err != nil {
			return 0, err
		}
		...
		avgPricePoolAmt = poolValue / fnoutil.Amount(poolSize)
	...
	if t.priceMode == AvgPriceVWAPMode || t.priceMode == AvgPriceDualMode {
		...
		for i := 0; i < vwapReqTries; i++ {
			ticketVWAP, err = t.fnodChainSvr.TicketVWAP(&startVWAPHeight,
				&endVWAPHeight)
