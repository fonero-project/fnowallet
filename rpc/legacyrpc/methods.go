...
import (
	...
	"github.com/fonero-project/fnowallet/chain"
	"github.com/fonero-project/fnowallet/errors"
	"github.com/fonero-project/fnowallet/internal/helpers"
	"github.com/fonero-project/fnowallet/p2p"
	ver "github.com/fonero-project/fnowallet/version"
	"github.com/fonero-project/fnowallet/wallet"
	"github.com/fonero-project/fnowallet/wallet/txrules"
...
var handlers = map[string]handler{
	...
	// Reference methods which can't be implemented by fnowallet due to
...
func unimplemented(*Server, interface{}) (interface{}, error) {
	return nil, &fnojson.RPCError{
		Code:    fnojson.ErrRPCUnimplemented,
		Message: "Method unimplemented",
	}
}

// unsupported handles a standard bitcoind RPC request which is
// unsupported by fnowallet due to design differences.
func unsupported(*Server, interface{}) (interface{}, error) {
	return nil, &fnojson.RPCError{
		Code:    -1,
		Message: "Request unsupported by fnowallet",
	}
}
...
// context.
type lazyHandler func() (interface{}, *fnojson.RPCError)
...
func lazyApplyHandler(s *Server, request *fnojson.Request) lazyHandler {
	handlerData, ok := handlers[request.Method]
	if !ok {
		return func() (interface{}, *fnojson.RPCError) {
			...
			if err != nil {
				return nil, rpcErrorf(fnojson.ErrRPCClientNotConnected, "RPC passthrough requires fnod RPC synchronization")
			}
	...
	return func() (interface{}, *fnojson.RPCError) {
		cmd, err := fnojson.UnmarshalCmd(request)
		if err != nil {
			return nil, fnojson.ErrRPCInvalidRequest
		}
...
func makeResponse(id, result interface{}, err error) fnojson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return fnojson.Response{
			ID:    idPtr,
...
func accountAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.AccountAddressIndexCmd)
	...
	case udb.InternalBranch:
		return intChild, nil
	default:
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "invalid branch %v", cmd.Branch)
	}
}
...
func accountSyncAddressIndex(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.AccountSyncAddressIndexCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if index >= hdkeychain.HardenedKeyStart {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter,
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*fnoutil.AddressSecpPubKey, len(keys))
	...
	for i, a := range keys {
		...
		switch addr := a.(type) {
		case *fnoutil.AddressSecpPubKey:
		...
		default:
			...
			if fnoec.SignatureType(pubKey.GetType()) != fnoec.STEcdsaSecp256k1 {
				return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter,
					"only secp256k1 pubkeys are currently supported")
			}
			pubKeyAddr, err := fnoutil.NewAddressSecpPubKey(
				pubKey.Serialize(), w.ChainParams())
			if err != nil {
				return nil, rpcError(fnojson.ErrRPCInvalidAddressOrKey, err)
			}
...
func addMultiSigAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]fnoutil.Address, len(cmd.Keys))
	...
	err = n.LoadTxFilter(context.TODO(), false, []fnoutil.Address{p2shAddr}, nil)
	if err != nil {
		return nil, err
	}
...
func addTicket(s *Server, icmd interface{}) (interface{}, error) {
	...
	if err != nil {
		return nil, rpcError(fnojson.ErrRPCDeserialization, err)
	}
...
func consolidate(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ConsolidateCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	var changeAddr fnoutil.Address
	if cmd.Address != nil {
		if *cmd.Address != "" {
...
func createMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.CreateMultisigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	address, err := fnoutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		return nil, err
	}

	return fnojson.CreateMultiSigResult{
...
func dumpPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.DumpPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func generateVote(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GenerateVoteCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		return nil, rpcError(fnojson.ErrRPCDecodeHexString, err)
	}
	...
	resp := &fnojson.GenerateVoteResult{
		Hex: b.String(),
	}
	return resp, nil
}
...
func getAddressesByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetAddressesByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if minConf < 0 {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "minconf must be non-negative")
	}
	...
	result := fnojson.GetBalanceResult{
	...
	if accountName == "*" {
		...
		var (
			totImmatureCoinbase fnoutil.Amount
			totImmatureStakegen fnoutil.Amount
			totLocked           fnoutil.Amount
			totSpendable        fnoutil.Amount
			totUnconfirmed      fnoutil.Amount
			totVotingAuthority  fnoutil.Amount
			cumTot              fnoutil.Amount
		)

		balancesLen := uint32(len(balances))
		result.Balances = make([]fnojson.GetAccountBalanceResult, balancesLen)
		...
		for _, bal := range balances {
			...
			if err != nil {
				...
				if errors.Is(errors.NotExist, err) {
					return nil, rpcError(fnojson.ErrRPCInternal.Code, err)
				}
				return nil, err
			}
			...
			json := fnojson.GetAccountBalanceResult{
...
func getBestBlock(s *Server, icmd interface{}) (interface{}, error) {
	...
	result := &fnojson.GetBestBlockResult{
		Hash:   hash.String(),
		Height: int64(height),
	}
	return result, nil
...
func getInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	var spendableBalance fnoutil.Amount
	for _, balance := range balances {
		spendableBalance += balance.Spendable
	}

	info := &fnojson.InfoWalletResult{
		Version:         ver.Integer,
...
func decodeAddress(s string, params *chaincfg.Params) (fnoutil.Address, error) {
	...
	if len(s) == 66 || len(s) == 130 {
		...
		pubKeyAddr, err := fnoutil.NewAddressSecpPubKey(pubKeyBytes,
			params)
		if err != nil {
			return nil, err
		}
	...
	addr, err := fnoutil.DecodeAddress(s)
	if err != nil {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidAddressOrKey,
...
func getAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
// appear in the blockchain, or any tx that has arrived in the fnod mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return fnojson.ErrRPCWalletKeypoolRanOut if that happens).
func getAccountAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetAccountAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getUnconfirmedBalance(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetUnconfirmedBalanceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func importPrivKey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ImportPrivKeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	wif, err := fnoutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidAddressOrKey, "WIF decode failed: %v", err)
	}
	if !wif.IsForNet(w.ChainParams()) {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidAddressOrKey, "key is not intended for %s", w.ChainParams().Name)
	}
...
func importScript(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ImportScriptCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if len(rs) == 0 {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "empty script")
	}
...
// keypoolRefill handles the keypoolrefill command.  fnowallet generates
...
func createNewAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.CreateNewAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		if errors.Is(errors.Locked, err) {
			return nil, rpcErrorf(fnojson.ErrRPCWalletUnlockNeeded, "creating new accounts requires an unlocked wallet")
		}
		return nil, err
	}
	return nil, nil
...
func renameAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.RenameAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getMultisigOutInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetMultisigOutInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	result := &fnojson.GetMultisigOutInfoResult{
...
func getNewAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetNewAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.GapPolicy != nil {
		...
		default:
			return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "unknown gap policy %q", *cmd.GapPolicy)
		}
	}
...
func getRawChangeAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetRawChangeAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getMasterPubkey(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetMasterPubkeyCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func getStakeInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &fnojson.GetStakeInfoResult{
...
func getTickets(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetTicketsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	return &fnojson.GetTicketsResult{Hashes: ticketHashStrs}, nil
}

// getTransaction handles a gettransaction request by returning details about
// a single transaction saved by wallet.
func getTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.GetTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if errors.Is(errors.NotExist, err) {
		return nil, rpcErrorf(fnojson.ErrRPCNoTxInfo, "no information for transaction")
	} else if err != nil {
		return nil, err
	}
	...
	ret := fnojson.GetTransactionResult{
		TxID:            cmd.Txid,
		Hex:             b.String(),
	...
	var (
		debitTotal  fnoutil.Amount
		creditTotal fnoutil.Amount
		fee         fnoutil.Amount
		negFeeF64   float64
	)
	...
	if len(txd.Debits) == len(txd.MsgTx.TxIn) {
		var outputTotal fnoutil.Amount
		for _, output := range txd.MsgTx.TxOut {
			outputTotal += fnoutil.Amount(output.Value)
		}
	...
	ret.Details = make([]fnojson.GetTransactionDetailsResult, len(details))
	for i, d := range details {
		ret.Details[i] = fnojson.GetTransactionDetailsResult{
			Account:           d.Account,
			Address:           d.Address,
			Amount:            d.Amount,
			Category:          d.Category,
...
func getVoteChoices(s *Server, icmd interface{}) (interface{}, error) {
	...
	resp := &fnojson.GetVoteChoicesResult{
		Version: version,
		Choices: make([]fnojson.VoteChoice, len(agendas)),
	}
	...
	for i := range choices {
		resp.Choices[i] = fnojson.VoteChoice{
...
func help(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.HelpCmd)
	// TODO: The "help" RPC should use a HTTP POST client when calling down to
	// fnod for additional help methods.  This avoids including websocket-only
	...
	return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "no help for method %q", *cmd.Command)
}

// listAccounts handles a listaccounts request by returning a map of account
// names to their balances.
func listAccounts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListAccountsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listReceivedByAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListReceivedByAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	jsonResults := make([]fnojson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, fnojson.ListReceivedByAccountResult{
...
func listReceivedByAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListReceivedByAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	ret := make([]fnojson.ListReceivedByAddressResult, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = fnojson.ListReceivedByAddressResult{
			Address:       address,
...
func listSinceBlock(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListSinceBlockCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if targetConf < 1 {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "target_confirmations must be positive")
	}
	...
	res := &fnojson.ListSinceBlockResult{
...
func listScripts(s *Server, icmd interface{}) (interface{}, error) {
	...
	listScriptsResultSIs := make([]fnojson.ScriptInfo, len(redeemScripts))
	for i, redeemScript := range redeemScripts {
		p2shAddr, err := fnoutil.NewAddressScriptHash(redeemScript,
			w.ChainParams())
		if err != nil {
			return nil, err
		}
		listScriptsResultSIs[i] = fnojson.ScriptInfo{
			Hash160:      hex.EncodeToString(p2shAddr.Hash160()[:]),
			Address:      p2shAddr.EncodeAddress(),
			RedeemScript: hex.EncodeToString(redeemScript),
		}
	}
	return &fnojson.ListScriptsResult{Scripts: listScriptsResultSIs}, nil
}

// listTransactions handles a listtransactions request by returning an
// array of maps with details of sent and recevied wallet transactions.
func listTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listAddressTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListAddressTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listAllTransactions(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListAllTransactionsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func listUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ListUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func lockUnspent(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.LockUnspentCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func purchaseTicket(s *Server, icmd interface{}) (interface{}, error) {
	...
	cmd := icmd.(*fnojson.PurchaseTicketCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	spendLimit, err := fnoutil.NewAmount(cmd.SpendLimit)
	if err != nil {
		return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
	}
	if spendLimit < 0 {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "negative spend limit")
	}
	...
	if cmd.MinConf != nil {
		minConf = int32(*cmd.MinConf)
		if minConf < 0 {
			return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "negative minconf")
		}
	}

	// Set ticket address if specified.
	var ticketAddr fnoutil.Address
	...
	if cmd.PoolAddress != nil {
		if *cmd.PoolAddress != "" {
			...
			// the pool after.
			if cmd.PoolFees == nil {
				return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "pool address set without pool fee")
			}
			poolFee = *cmd.PoolFees
			if !txrules.ValidPoolFeeRate(poolFee) {
				return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "pool fee percentage %v", poolFee)
			}
		}
	}
	...
	if cmd.TicketFee != nil {
		ticketFee, err = fnoutil.NewAmount(*cmd.TicketFee)
		if err != nil {
			return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
		}
	}
...
// and amounts.
func makeOutputs(pairs map[string]fnoutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
...
// All errors are returned in fnojson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]fnoutil.Amount, account uint32, minconf int32) (string, error) {
	...
	if err != nil {
		...
		if errors.Is(errors.InsufficientBalance, err) {
			return "", rpcError(fnojson.ErrRPCWalletInsufficientFunds, err)
		}
		return "", err
	}
...
func redeemMultiSigOut(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.RedeemMultiSigOutCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	srtc := &fnojson.SignRawTransactionCmd{
		RawTx:    b.String(),
		Inputs:   &rtis,
		PrivKeys: &[]string{},
		Flags:    &sigHashAll,
	...
	srtTyped := signedTxResult.(fnojson.SignRawTransactionResult)
	return fnojson.RedeemMultiSigOutResult(srtTyped), nil
}
...
func redeemMultiSigOuts(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.RedeemMultiSigOutsCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	p2shAddr, ok := addr.(*fnoutil.AddressScriptHash)
	if !ok {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "address is not P2SH")
	}
	...
	itr := uint32(0)
	rmsoResults := make([]fnojson.RedeemMultiSigOutResult, len(msos))
	for i, mso := range msos {
		if itr > max {
			break
		}

		rmsoRequest := &fnojson.RedeemMultiSigOutCmd{
		...
		redeemResultTyped := redeemResult.(fnojson.RedeemMultiSigOutResult)
		rmsoResults[i] = redeemResultTyped

		itr++
	}

	return fnojson.RedeemMultiSigOutsResult{Results: rmsoResults}, nil
}
...
func rescanWallet(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.RescanWalletCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func stakePoolUserInfo(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.StakePoolUserInfoCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	userAddr, err := fnoutil.DecodeAddress(cmd.User)
	if err != nil {
		return nil, err
	}
	...
	resp := new(fnojson.StakePoolUserInfoResult)
	resp.Tickets = make([]fnojson.PoolUserTicket, 0, len(spui.Tickets))
	resp.InvalidTickets = make([]string, 0, len(spui.InvalidTickets))
	for _, ticket := range spui.Tickets {
		var ticketRes fnojson.PoolUserTicket
...
func ticketsForAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.TicketsForAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	addr, err := fnoutil.DecodeAddress(cmd.Address)
	if err != nil {
		return nil, err
	}
	...
	return fnojson.TicketsForAddressResult{Tickets: ticketHashStrs}, nil
}
...
func sendFrom(s *Server, icmd interface{}) (interface{}, error) {
	...
	if !isNilOrEmpty(cmd.Comment) || !isNilOrEmpty(cmd.CommentTo) {
		return nil, rpcErrorf(fnojson.ErrRPCUnimplemented, "transaction comments are unsupported")
	}
	...
	if cmd.Amount < 0 {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "negative amount")
	}
	minConf := int32(*cmd.MinConf)
	if minConf < 0 {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "negative minconf")
	...
	amt, err := fnoutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
	}
	pairs := map[string]fnoutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(s *Server, icmd interface{}) (interface{}, error) {
	...
	// Recreate address/amount pairs, using fnoutil.Amount.
	pairs := make(map[string]fnoutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := fnoutil.NewAmount(v)
		if err != nil {
			return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
		}
		pairs[k] = amt
...
func sendToAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.SendToAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func sendToMultiSig(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.SendToMultiSigCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	account := uint32(udb.DefaultAccountNum)
	amount, err := fnoutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
	}
	nrequired := int8(*cmd.NRequired)
	minconf := int32(*cmd.MinConf)
	pubkeys := make([]*fnoutil.AddressSecpPubKey, len(cmd.Pubkeys))
	...
	result := &fnojson.SendToMultiSigResult{
...
func setTicketFee(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.SetTicketFeeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.Fee < 0 {
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "negative fee")
	}

	incr, err := fnoutil.NewAmount(cmd.Fee)
	if err != nil {
		return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
	}
...
func setTxFee(s *Server, icmd interface{}) (interface{}, error) {
	...
	relayFee, err := fnoutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
	}
...
func setVoteChoice(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.SetVoteChoiceCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func signMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.SignMessageCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func signRawTransaction(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.SignRawTransactionCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	case "ssrtx": // Special case of SigHashAll
		hashType = txscript.SigHashAll
	default:
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "invalid sighash flag")
	}

	// TODO: really we probably should look these up with fnod anyway to
	// make sure that they match the blockchain if present.
	inputs := make(map[wire.OutPoint][]byte)
	scripts := make(map[string][]byte)
	var cmdInputs []fnojson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := fnoutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, err
			}
	...
	// querying fnod with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*fnoutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*fnoutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := fnoutil.DecodeWIF(key)
			if err != nil {
				return nil, rpcError(fnojson.ErrRPCDeserialization, err)
			}

			if !wif.IsForNet(w.ChainParams()) {
				return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "key intended for different network")
			}

			var addr fnoutil.Address
			switch wif.DSA() {
			case fnoec.STEcdsaSecp256k1:
				addr, err = fnoutil.NewAddressSecpPubKey(wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
				}
			case fnoec.STEd25519:
				addr, err = fnoutil.NewAddressEdwardsPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
			...
			case fnoec.STSchnorrSecp256k1:
				addr, err = fnoutil.NewAddressSecSchnorrPubKey(
					wif.SerializePubKey(),
					w.ChainParams())
				if err != nil {
					return nil, err
	...
	for outPoint, resp := range requested {
		...
		if err != nil {
			return nil, errors.E(errors.Op("fnod.jsonrpc.gettxout"), err)
		}
	...
	signErrors := make([]fnojson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, fnojson.SignRawTransactionError{
	...
	return fnojson.SignRawTransactionResult{
		Hex:      b.String(),
...
func startAutoBuyer(s *Server, icmd interface{}) (interface{}, error) {
	...
	if cmd.VotingAddress != nil {
		var votingAddress fnoutil.Address
	...
	var poolAddress fnoutil.Address
	if cmd.PoolAddress != nil {
		if *cmd.PoolAddress != "" {
	...
	case poolFees == 0 && poolAddress != nil:
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "pooladdress set without poolfees")
	case poolFees != 0 && poolAddress == nil:
		return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "poolfees set without pooladdress")
	case poolFees != 0 && poolAddress != nil:
		if !txrules.ValidPoolFeeRate(poolFees) {
			return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "invalid poolfees %v", poolFees)
		}
	}
...
func makeScriptChangeSource(address string, version uint16) (*scriptChangeSource, error) {
	destinationAddress, err := fnoutil.DecodeAddress(address)
	if err != nil {
		return nil, err
	}
...
func sweepAccount(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.SweepAccountCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if cmd.FeePerKb != nil {
		var err error
		feePerKb, err = fnoutil.NewAmount(*cmd.FeePerKb)
		if err != nil {
			return nil, rpcError(fnojson.ErrRPCInvalidParameter, err)
		}
	}
	...
	if err != nil {
		if errors.Is(errors.InsufficientBalance, err) {
			return nil, rpcError(fnojson.ErrRPCWalletInsufficientFunds, err)
		}
		return nil, err
	}
	...
	res := &fnojson.SweepAccountResult{
...
func validateAddress(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.ValidateAddressCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}

	result := fnojson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.VerifyMessageCmd)
	...
	switch a := addr.(type) {
	case *fnoutil.AddressSecpPubKey:
	case *fnoutil.AddressPubKeyHash:
		if a.DSA(a.Net()) != fnoec.STEcdsaSecp256k1 {
			goto WrongAddrKind
		}
	default:
		goto WrongAddrKind
...
WrongAddrKind:
	return nil, rpcErrorf(fnojson.ErrRPCInvalidParameter, "address must be secp256k1 P2PK or P2PKH")
}
...
func version(s *Server, icmd interface{}) (interface{}, error) {
	var resp map[string]fnojson.VersionResult
	n, _ := s.walletLoader.NetworkBackend()
	chainClient, err := chain.RPCClientFromBackend(n)
	if err == nil {
		var err error
		...
		resp = make(map[string]fnojson.VersionResult)
	}

	resp["fnowalletjsonrpcapi"] = fnojson.VersionResult{
...
func walletInfo(s *Server, icmd interface{}) (interface{}, error) {
	...
	return &fnojson.WalletInfoResult{
...
func walletPassphrase(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.WalletPassphraseCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
...
func walletPassphraseChange(s *Server, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*fnojson.WalletPassphraseChangeCmd)
	w, ok := s.walletLoader.LoadedWallet()
	if !ok {
		return nil, errUnloadedWallet
	}
	...
	if err != nil {
		if errors.Is(errors.Passphrase, err) {
			return nil, rpcErrorf(fnojson.ErrRPCWalletPassphraseIncorrect, "incorrect passphrase")
		}
		return nil, err
	}
	return nil, nil
...
func decodeHexStr(hexStr string) ([]byte, error) {
	...
	if err != nil {
		return nil, rpcErrorf(fnojson.ErrRPCDecodeHexString, "hex string decode failed: %v", err)
	}
	return decoded, nil
}
