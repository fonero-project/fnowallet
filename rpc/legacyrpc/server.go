...
// method.  This may be a request that is handled directly by fnowallet, or
// a chain server request that is handled by passing the request down to fnod.
//
...
func (s *Server) handlerClosure(ctx context.Context, request *fnojson.Request) lazyHandler {
...
// but this is only implemented here to work around an oddity with fnojson,
...
func (s *Server) invalidAuth(req *fnojson.Request) bool {
	cmd, err := fnojson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*fnojson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := fnojson.MarshalResponse(req.Jsonrpc, req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := fnojson.MarshalResponse(req.Jsonrpc, req.ID, nil, fnojson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response to client %s: %v",
				r.RemoteAddr, err)
	...
	case "stop":
		...
		stop = true
		res = "fnowallet stopping"
	default:
		res, jsonErr = s.handlerClosure(ctx, &req)()
	}

	// Marshal and send.
	mresp, err := fnojson.MarshalResponse(req.Jsonrpc, req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response to client %s: %v",
			r.RemoteAddr, err)
