...
the management of the wallet and its connection to the Fonero network.  It has
...
dynamically start the fnod RPC client.  This RPC client is used for wallet
syncing and publishing transactions to the Fonero network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default fnod RPC port of the
  active Fonero network.
...
- `string public_key`: The public key encoded as a string in the Fonero encoding
  format.
...
the Fonero network.  If the transaction spends any of the wallet's unspent
...
notifications passed from the JSON RPC of fnod. Right now, wallet block processing
