...
var certificateFile = filepath.Join(fnoutil.AppDataDir("fnowallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", fnoutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/fnowallet/rpc --cpp_out=. --grpc_out=. \
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by fnowallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.fnowallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\fnowallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by fnowallet).
...
if (os.platform() == 'win32') {
  ...
  certPath = path.join(process.env.HOME, '.fnowallet', 'rpc.cert');
}
...
def main():
    ...
    if platform.system() == 'Windows':
        ...
        cert_file_path = os.path.join(os.environ['HOME'], '.fnowallet', 'rpc.cert')
