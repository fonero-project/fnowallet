...
func (b *rpcBackend) GetBlocks(ctx context.Context, blockHashes []*chainhash.Hash) ([]*wire.MsgBlock, error) {
	const op errors.Op = "fnod.jsonrpc.getblock"
...
func (b *rpcBackend) GetHeaders(ctx context.Context, blockLocators []*chainhash.Hash, hashStop *chainhash.Hash) ([]*wire.BlockHeader, error) {
	const op errors.Op = "fnod.jsonrpc.getheaders"
...
func (b *rpcBackend) LoadTxFilter(ctx context.Context, reload bool, addrs []fnoutil.Address, outpoints []wire.OutPoint) error {
	const op errors.Op = "fnod.jsonrpc.loadtxfilter"
...
func (b *rpcBackend) PublishTransactions(ctx context.Context, txs ...*wire.MsgTx) error {
	const op errors.Op = "fnod.jsonrpc.sendrawtransaction"
...
func (b *rpcBackend) Rescan(ctx context.Context, blocks []chainhash.Hash, r wallet.RescanSaver) error {
	const op errors.Op = "fnod.jsonrpc.rescan"
...
func (b *rpcBackend) StakeDifficulty(ctx context.Context) (fnoutil.Amount, error) {
	const op errors.Op = "fnod.jsonrpc.getstakedifficulty"

	r, err := b.rpcClient.GetStakeDifficulty()
	if err != nil {
		return 0, errors.E(op, err)
	}
	amount, err := fnoutil.NewAmount(r.NextStakeDifficulty)
	if err != nil {
		return 0, errors.E(op, err)
	}
	return amount, nil
